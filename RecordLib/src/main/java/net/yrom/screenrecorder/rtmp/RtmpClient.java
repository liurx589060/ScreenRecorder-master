...
public class RtmpClient {
    ...
    public void open(String url, boolean isPublishMode) throws RtmpIOException {
        ...
        Log.e("yy","" + rtmpPointer);
        openResult = nativeOpen(url, isPublishMode, rtmpPointer);
        Log.e("yy","open result=" + openResult);
        if (openResult != OPEN_SUCCESS) {
            throw new RtmpIOException(openResult);
        }
    }
     ...
     * @throws IOException if connection is not opened or connection to server is lost
     */
    public int write(long rtmpPointer, byte[] data, int size, int type, int ts) throws IOException {
        if(openResult != OPEN_SUCCESS) return -1;
        return nativeWrite(this.rtmpPointer,data,size,type,ts);
    }

    //private native int nativeWrite(byte[] data, long rtmpPointer) throws IOException;
    public static native int nativeWrite(long rtmpPointer, byte[] data, int size, int type, int ts);
     ...
     */
    public void close() {
        if(openResult != OPEN_SUCCESS) return;
